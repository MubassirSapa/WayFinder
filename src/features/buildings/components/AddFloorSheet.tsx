...
export function AddFloorSheet({
  ...
  return (
    <div className="fixed inset-0 z-50">
      <button
        type="button"
        aria-label={BUILDINGS_CLIENT.CLOSE}
      ...
      <div className="absolute inset-y-0 right-0 flex w-full max-w-md flex-col border-l border-border bg-card shadow-xl animate-in fade-in-0 slide-in-from-right duration-200">
        <div className="flex items-center justify-between gap-4 border-b border-border px-5 py-5">
          <div>
            <h2 className="font-heading text-lg font-semibold">{BUILDINGS_CLIENT.CREATE_FLOOR_SHEET_TITLE}</h2>
            {organizationName ? (
        ...
        >
          <div className="flex-1 space-y-5 overflow-y-auto px-5 py-6">
            <div className="space-y-2">
              <Label htmlFor="floor-name">{BUILDINGS_CLIENT.CREATE_FLOOR_FIELD_NAME_LABEL}</Label>
              <Input
                id="floor-name"
                value={name}
                onChange={(event) => setName(event.target.value)}
                placeholder={BUILDINGS_CLIENT.CREATE_FLOOR_FIELD_NAME_PLACEHOLDER}
                className="h-10"
                disabled={isPending}
                autoFocus
              />
            ...
            <div className="space-y-2">
              <Label htmlFor="floor-level">{BUILDINGS_CLIENT.FIELD_LEVEL_LABEL}</Label>
            ...
            <div className="flex items-center justify-between gap-4 rounded-xl border border-border bg-background/40 p-4">
              <div>
                <p className="text-sm font-medium">{BUILDINGS_CLIENT.FIELD_PUBLISH_TITLE}</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {publish ? BUILDINGS_CLIENT.FIELD_PUBLISH_ON : BUILDINGS_CLIENT.FIELD_PUBLISH_OFF}
                </p>
              </div>
