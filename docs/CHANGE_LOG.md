...
- Added direct-to-Cloudflare-R2 client uploads (`src/plugins/storage/`, `src/lib/uploads/uploadMediaClientSide.ts`): organization logo, building logo, avatar, and floor reference image uploads now go straight from the browser to R2 instead of through our server, avoiding Vercel's request body size limit and the extra hop. Falls back to local disk storage if R2 env vars aren't set. Org logo, building logo, and avatar uploads now happen as soon as a file is picked instead of on form submit. Uploaded files are renamed client-side to a random unique name instead of keeping the original filename, avoiding collisions and sidestepping sanitizing/URL-encoding whatever the original name held. See `docs/technical/MEDIA_STORAGE.md`.
