...
- Added new dashboard pages, each gated by the same access functions that already protect their data: `/dashboard` itself now shows the organization (name/type/logo) plus every building the signed-in user can access — every building in the organization for an owner/manager, or just the buildings they're assigned to for a member — instead of jumping straight into one building's floor list; `/dashboard/buildings/[buildingId]` (a building's own record — name/address/contact/logo — editable by owner/manager, read-only for an assigned member; plus its floor list, add-floor, and per-floor publish toggle), `/dashboard/buildings/[buildingId]/floors/[floorId]` (floor metadata edit — name, level, dimensions, meters-per-pixel, status — for any role with access to that building, including members, since being assigned to a building is what grants working access to its content), `/dashboard/organization` (edit organization name/type/logo, owner/manager only), `/dashboard/users` (list org users, create a manager/member with an initial password, change role/building assignment, remove a non-owner user, owner/manager only), and `/dashboard/profile` (edit own name and avatar, any role). Each page follows the existing `services/server` → `actions/server` layering and authorizes mutations by passing the real signed-in user through the Local API with `overrideAccess: false`, rather than an `overrideAccess: true` bypass — this closes a real gap in floor creation/publish-toggle, which previously had no building-scoped access check at all beyond "is logged in."
- Added a shared `AppTopbar` (avatar menu with profile link, theme toggle, and logout; a text-only wordmark instead of the icon+"Admin" badge header) rendered by a new `/dashboard` layout, so every page above shares one consistent header instead of each page building its own — the `/editor` route is unaffected since the layout only wraps `/dashboard/**`.
