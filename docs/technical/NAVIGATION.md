...
There are five separate places a user can change the active floor: the header floor select, `MapViewerSidebar`'s floor list, double-clicking/double-tapping a canvas connector, clicking a `RoutePanel` segment row, and `FloorHopIndicator`'s "Continue via stairs/elevator to Floor N" button. All five write to the same `activeFloorId`/`activeSegmentIndex` pair in the store, which is what keeps the header, the in-canvas breadcrumb (`MapViewerToolbar`), the route panel's "you're here" highlight, and the drawn route line (`routePointsForActiveFloor` in `MapViewerShell`, which only draws when `activeSegment.floorId === activeFloorId`) all in agreement. The header/sidebar/connector paths go through a shared `goToFloor(floorId)` helper in `MapViewerShell` that also looks up and syncs `activeSegmentIndex` to whichever segment (if any) matches the new floor; the route panel row and floor-hop button already know the exact segment index, so they set it directly instead.
