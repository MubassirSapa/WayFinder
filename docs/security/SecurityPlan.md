...
**Update — T1/F1 status: HALF-FIXED; remainder is an accepted risk, knowingly still exploitable via the app's own editor UI.** `Floors`, `MapNodes`, `MapObjects`, and `PathEdges` now define real `access` blocks (`buildingContentRead`/`buildingContentCreate`/`buildingContentUpdateDelete`), which correctly protects the raw REST/GraphQL surface this paragraph tested. But the app's own map-editor server actions (`src/features/map-editor/core/services/server/{floor,object,node,edge}-pl.adapter.ts`) still call Payload with `overrideAccess: true` (explicit in the floor adapter, implicit-by-omission — Payload's Local API default — in the object/node/edge adapters), and none of the corresponding server actions add a manual ownership check. This means the real `/editor/{floorId}` page, used by real signed-in users, still lets any authenticated user from any organization view and fully edit/delete any other organization's map data — see the updated T1 row above for the exact evidence. The fact that the vulnerable code runs server-side does **not** make this safe: the attack surface is a client navigating to another org's floor id through the real UI, and `overrideAccess: true` is exactly what skips the org check the collection's own `access` rules already do correctly. This is **not resolved** — it is accepted as an open, exploitable risk for this submission rather than fixed, and remains the highest-priority item before any real deployment.
...
|---|---|---|---|---|---|
| F1 | Two-layer authorization gap on map data: (a) `Floors`/`MapNodes`/`MapObjects`/`PathEdges` define no `access` rules, so any authenticated user (any org/role) can read/write any organization's records via raw REST/GraphQL; (b) the app's own server actions bypass access control entirely via Payload's Local API `overrideAccess` behavior, with no manual organization-ownership check in its place | **Part (a) fixed; part (b) — accepted risk, intentionally deferred for this submission — Critical** | Two changes needed together: (1) add `access` blocks to all four collections scoping create/update/delete to the record's owning organization (mirroring `Organizations.ts`) — **confirmed done**, all four collections now use `access.buildingContentRead/Create/UpdateDelete`; (2) update the server actions/adapters to pass `overrideAccess: false` plus the real `req`/`user`, or add an explicit "does this record belong to the caller's org" check before every mutation — **confirmed NOT done, and knowingly left this way for this submission**: `floor-pl.adapter.ts` still explicitly sets `overrideAccess: true`; `object-pl.adapter.ts`, `node-pl.adapter.ts`, `edge-pl.adapter.ts` omit `overrideAccess` entirely (defaults to `true`); none of `floor-actions.ts`/`object-actions.ts`/`node-actions.ts`/`edge-actions.ts` check the caller's organization before forwarding a client-supplied id. **This is not mitigated by the fix living in server-side code** — the bug is reachable by any authenticated user through the real `/editor/{floorId}` UI (they only need to know or guess another org's floor id, not read any source), and `overrideAccess: true` is exactly what causes the server to skip the organization check that part (a)'s collection rules would otherwise enforce. The team is accepting this as a known gap for the capstone submission rather than treating it as fixed; it remains the single most important open item in this plan and the first thing to fix before any real deployment | Backend dev | Re-run §3.3: (a) direct REST/GraphQL calls as a user from a different org must return 403 — **passes now**; (b) as two different organizations' real signed-in users, open `/editor/{floorId}` for the other org's floor and attempt to edit/delete — **currently succeeds; not tested further, no fix planned for this submission** |
...
| F2 | `Media` collection: read is intentionally public (confirmed, `GET /api/media` → 200); create/update/delete correctly require *some* authenticated user but aren't scoped to org/role, and there's no file type/size limit on `upload: true` | **Partially fixed — High** | Add explicit `access.isLoggedIn` (or org-scoped) for create/update/delete for clarity/defense-in-depth, and add `upload: { mimeTypes: ["image/png","image/jpeg","image/jpg","image/webp","image/avif"], },` with a size limit. **`create: access.isLoggedIn` is confirmed added** (`src/collections/Media.ts`, with an in-code comment noting direct-to-R2 client uploads make this the real boundary for that path). **`mimeTypes: ["image/png","image/jpeg","image/jpg","image/webp","image/avif"]` is now added** to `upload` on `Media.ts` — Payload auto-generates a `mimeType` field validator from this, enforced server-side on doc creation even for the direct-to-R2 client-upload path (confirmed against `node_modules/payload/dist/uploads/getBaseFields.js`). The global `upload.limits.fileSize` (`payload.config.ts`, backed by `MEDIA_MAX_FILE_SIZE_BYTES` = 5MB) was already enforced server-side in the signed-URL step; client-side pre-checks for both type and size now exist in all four upload forms (`BuildingForm`, `OrganizationForm`, `ProfileForm`, `UserInfoEditForm`) for immediate UX feedback, not as the security boundary. **`update`/`delete` still have no explicit access block** (fall back to Payload's default, still not org-scoped) — remains open, not addressed by this pass | Backend dev | Repeat upload test as an authenticated user with disallowed mime types/oversized files; expect rejection — **mime type and size now rejected server-side; update/delete cross-org scoping still open** |
| F3 | No lockout on repeated failed `/signin` attempts, and no throttling on `/forgot-password`; no dedicated DDoS mitigation beyond Cloudflare's default | **High — accepted risk, no fix planned** | `auth: { maxLoginAttempts: 5, lockTime: 10 * 60 * 1000 }` on `Users.ts` remains a known, cheap, built-in fix (Payload enforces account lockout natively, no Redis/Upstash required) but is **intentionally deferred**: the team is knowingly leaving `/signin` without lockout for this submission, scoped as a demo-only decision rather than a production posture. `/forgot-password` throttling and broader volumetric DDoS mitigation remain separately accepted risk as before (see §1.2 note on T3) | Backend dev | Not tested — no fix planned for this submission; revisit `maxLoginAttempts`/`lockTime` before any real deployment |
...
**Closure process:** each finding above becomes a tracked issue; once a fix lands, the listed re-test is re-run and the issue is only closed when the re-test passes. F4 is a blocker for any public/production deployment and should be resolved before end of Week 10; F2 is mostly closed (mimeTypes/size allowlist done, update/delete scoping still open). F1's server-action/adapter half, F3 in full (both the `/signin` lockout piece and the `/forgot-password`/DDoS portion), F5, F9 (verify only), and F10 are accepted risks with no fix planned for this submission (see §7) — they are tracked here, not treated as blockers, but F1 in particular is the first thing to fix before any real deployment. F6–F8 should be resolved before the next milestone demo.
...
|---|---|---|---|---|
| — | F1 | Add `access` blocks to `Floors`, `MapNodes`, `MapObjects`, `PathEdges` scoping create/update/delete to the owning organization's admin/user, **and** update the map-editor server actions/adapters to stop bypassing that via `overrideAccess: true`/default | N/A | **Half-done, remainder accepted risk**: collection `access` blocks shipped; the server-action/adapter half (`floor-pl.adapter.ts`, `object-pl.adapter.ts`, `node-pl.adapter.ts`, `edge-pl.adapter.ts`) is **intentionally deferred for this submission** — a deliberate scope decision, not an oversight — even though it's the higher-impact half, since it's exploitable through the real editor UI by any authenticated user. First item to fix before any real deployment |
| 2 | F2 | Restrict `Media` create/update/delete to logged-in users; add `mimeTypes` allowlist and a max upload size | Before Week 10 submission | **Mostly done**: `create` access explicitly gated; `mimeTypes` allowlist (server-side, enforced even for direct-to-R2 client uploads) and the `MEDIA_MAX_FILE_SIZE_BYTES` size limit are now in place, both with matching client-side pre-checks for UX. `update`/`delete` access scoping is still open |
...
| — | F3 | Add `maxLoginAttempts`/`lockTime` to `Users.ts` auth config (login-lockout portion only) | N/A | **Accepted risk — intentionally deferred for this submission** (demo-only decision, not a production posture). `/forgot-password` throttling and broader DDoS mitigation remain accepted risk as before |
...
**Accepted risks:**
- **F1 (map-editor server actions bypass access control)** — the server-action/adapter half is accepted as a known, exploitable gap for this submission, not something believed to be safe. Location in server-side code does not mitigate it: the vulnerable path is reached by any authenticated user (any org) through the real `/editor/{floorId}` UI, and `overrideAccess: true`/default is precisely what causes the server to skip the organization-ownership check that the collection's own `access` rules already implement correctly. The team is choosing to ship the capstone with this open rather than treating it as fixed, and it is the top priority before any real deployment.
- **F3 (rate limiting / DDoS)** is accepted in full for this submission, including the `/signin` lockout piece: `maxLoginAttempts`/`lockTime` is a known, essentially-free fix (Payload supports it natively) but the team is intentionally leaving it out for now — a deliberate demo-scope decision, not an oversight. `/forgot-password` throttling and dedicated DDoS mitigation beyond Cloudflare's default edge protection remain accepted for the same reason as before — Payload has no built-in equivalent for arbitrary endpoints, volumetric attacks already get a free baseline mitigation from the Cloudflare-in-front-of-Vercel architecture, and none of this blocks a safe demo of a student project with no real user base at risk. If the project continued past the capstone, `maxLoginAttempts`/`lockTime` would be the first thing added back.
