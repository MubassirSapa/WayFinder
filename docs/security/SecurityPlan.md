...
|---|---|---|---|---|---|---|
| T1 | Cross-tenant tampering: any authenticated user (any role/org) can create/update/delete another organization's floor/node/object/edge data, because `Floors`/`MapNodes`/`MapObjects`/`PathEdges` define no `access` rules and fall back to Payload's default check (any logged-in user, not scoped to the record's owner). Separately, the app's own dashboard/map-editor server actions bypass that check entirely via Payload's Local API `overrideAccess` behavior, with no manual ownership check in its place | Tampering | 3 | 3 | **9 – Critical** | Authorization testing (§3), API fuzzing |
...
- Roles: `admin` / `user` (`src/collections/constants/roles.ts`), enforced via `isAdmin` / `isAdminOrSelf` (`src/collections/access/index.ts`).
...
|---|---|---|---|---|---|
| F1 | Two-layer authorization gap on map data: (a) `Floors`/`MapNodes`/`MapObjects`/`PathEdges` define no `access` rules, so any authenticated user (any org/role) can read/write any organization's records via raw REST/GraphQL — anonymous access is correctly blocked (verified, see §3.3); (b) the app's own server actions (`floorEditorActions.ts`, `dashboard-pl.adapter.ts`) bypass access control entirely via Payload's Local API `overrideAccess` (explicit `true` in some calls, default `true` in others), with no manual organization-ownership check in its place | **Critical** | Two changes needed together: (1) add `access` blocks to all four collections scoping create/update/delete to the record's owning organization (mirroring `Organizations.ts`); (2) update the server actions/adapters to pass `overrideAccess: false` plus the real `req`/`user`, or add an explicit "does this record belong to the caller's org" check before every mutation | Backend dev | Re-run §3.3: (a) direct REST/GraphQL calls as a user from a different org must return 403; (b) calling `toggleFloorStatusAction`/`updateMapObject` etc. with another org's id must be rejected, not silently succeed |
...
| F2 | `Media` collection: read is intentionally public (confirmed, `GET /api/media` → 200); create/update/delete correctly require *some* authenticated user (confirmed, anon `POST /api/media` → 403) but aren't scoped to org/role, and there's no file type/size limit on `upload: true` | **High** | Add explicit `access.isLoggedIn` (or org-scoped) for create/update/delete for clarity/defense-in-depth, and add `upload: { mimeTypes: ["image/png","image/jpeg","image/webp"], },` with a size limit | Backend dev | Repeat upload test as an authenticated user with disallowed mime types/oversized files; expect rejection |
...
|---|---|---|---|---|
| 1 | F1 | Add `access` blocks to `Floors`, `MapNodes`, `MapObjects`, `PathEdges` scoping create/update/delete to the owning organization's admin/user | Before Week 10 submission | Planned |
| 2 | F2 | Restrict `Media` create/update/delete to logged-in users; add `mimeTypes` allowlist and a max upload size | Before Week 10 submission | Planned |
